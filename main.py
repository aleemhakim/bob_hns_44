...
class HSDScheduler():
    def __init__(self):
        ...
        isAuth = False
        try:
            rs = self.client.connect(hostname='69.164.214.99', port=22, username='zahoor', password='H155P45sw3rd!')
            print(rs)
            isAuth = True
    ...
    def searchAndUploadData(self, rows):
        ...
        try:
            ...
            # row['daysuntilexpire'])
            query = '''
            SELECT Name,WalletID FROM (VALUES {search_rows} ) V(Name,WalletID) EXCEPT SELECT Name,WalletID FROM Names
            Insert into Names(Name,WalletID,Decoded_Punycode,Status,Reserved,HoursUntilBidding,HoursUntilReveal,HoursUntilClose,DaysUntilExpire) 
