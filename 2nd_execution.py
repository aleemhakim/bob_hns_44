...
class HSDScheduler():
    def __init__(self):
        ...
        if(len(list_of_tables) == 0):
          self.mycursor.execute("CREATE TABLE Domains(Name VARCHAR(255),Status VARCHAR(255))")
    ...
    def start(self):
        ...
        for index, row in inputDf.iterrows():
            ...
            continue
            input("check out the state here")
            print("thi")
            # for "OPENING" state
            if state == '"OPENING"':
    ...
    def searchAndUploadData(self, rows):
        ...
        try:
            ...
            print(data_to_be_inserted)
            query = '''
            SELECT Name,WalletID FROM (VALUES {search_rows} ) V(Name,WalletID) EXCEPT SELECT Name,WalletID FROM domains
            Insert into domains(Name,WalletID,Decoded_Punycode,Status,Reserved,HoursUntilBidding,HoursUntilReveal,HoursUntilClose,DaysUntilExpire) 
            SELECT Name,WalletID,Decoded_Punycode,Status,Reserved,HoursUntilBidding,HoursUntilReveal,HoursUntilClose,DaysUntilExpire 
            FROM (VALUES{data_to_be_inserted}) V(Name,WalletID,Decoded_Punycode,Status,Reserved,HoursUntilBidding,HoursUntilReveal,HoursUntilClose,DaysUntilExpire) 
            EXCEPT SELECT Name,WalletID,Decoded_Punycode,Status,Reserved,HoursUntilBidding,HoursUntilReveal,HoursUntilClose,DaysUntilExpire
            FROM domains
            '''.format(search_rows=data, data_to_be_inserted=data_to_be_inserted)
